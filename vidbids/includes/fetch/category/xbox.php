...
$sql = "SELECT `item_auction`.`id`, `item_auction`.`item_id`, `item_auction`.`current_bid`, `item_auction`.`timestamp` FROM `item_auction`, `item` WHERE `item_auction`.`item_id` = `item`.`id` AND `item`.`category` = 'xbox' ORDER BY bid_count DESC LIMIT 6"; // sql selecting popular auctions
...
if($num_rows > 0){
	while($row = mysqli_fetch_assoc($query)){
		...
		array_push($xbox, $auction);
	}
} else {
	echo 'none';
}

echo json_encode($xbox);
exit();
?>