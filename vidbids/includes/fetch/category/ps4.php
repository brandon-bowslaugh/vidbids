...
$sql = "SELECT `item_auction`.`id`, `item_auction`.`item_id`, `item_auction`.`current_bid`, `item_auction`.`timestamp` FROM `item_auction`, `item` WHERE `item_auction`.`item_id` = `item`.`id` AND `item`.`category` = 'ps4' ORDER BY bid_count DESC LIMIT 6"; // sql selecting popular auctions
...
if($num_rows > 0){
	while($row = mysqli_fetch_assoc($query)){
		...
		array_push($ps4, $auction);
	}
} else {
	echo 'none';
}

echo json_encode($ps4);
exit();
?>