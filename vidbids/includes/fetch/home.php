<?php
include_once('../connect/db_connect.php'); 
// include_once('../account/session.php');
// select popular, completed and relevant auctions
class return_data{
	//public $completed;
	public $popular;
	public $newer;
}
class auction{
	public $auction_id;
	public $name;
	public $bid_price;
	public $current_bid;
	public $timestamp;
	public $thumbnail;
}
/*
$sql = ""; // sql selecting completed auctions
$query = mysqli_query($db_connect, $sql);
$num_rows = mysqli_num_rows($query);
$completed=[];
if($num_rows > 0){
	while($row = mysqli_fetch_assoc($query)){
		$auction_id = $row['id'];
		$name = $row['name'];
		$bid_price = $row['bid_price'];
		$current_bid = $row['current_bid'];
		$timestamp = "COMPLETED";
		$winning_user = $row['username'];
		$auction = new auction;
		$auction -> auction_id = $auction_id;
		$auction -> name = $name;
		$auction -> bid_price = $bid_price;
		$auction -> current_bid = $current_bid;
		$auction -> timestamp = $timestamp;
		$auction -> winning_user = $winning_user;
		array_push($completed, $auction);
	}
} else {
	echo 'none';
}
*/
$sql = "SELECT id, item_id, current_bid, `timestamp` FROM item_auction ORDER BY bid_count DESC LIMIT 6"; // sql selecting popular auctions
$query = mysqli_query($db_connect, $sql);
$num_rows = mysqli_num_rows($query);
$popular=[];
if($num_rows > 0){
	while($row = mysqli_fetch_assoc($query)){
		$auction_id = $row['id'];
		$item_id = $row['item_id'];
		$current_bid = $row['current_bid'];
		$timestamp = $row['timestamp'];

		$sql = "SELECT name, bid_price, thumb FROM item WHERE id=$item_id";
		$query_2 = mysqli_query($db_connect, $sql);
		$row2 = mysqli_fetch_row($query_2);
		$name = $row2[0];
		$bid_price = $row2[1];
		$thumb = $row2[2];
		$auction = new auction;
		$auction -> auction_id = $auction_id;
		$auction -> item_id = $item_id;
		$auction -> name = $name;
		$auction -> bid_price = $bid_price;
		$auction -> current_bid = $current_bid;
		$auction -> timestamp = $timestamp;
		$auction -> thumbnail = $thumb;
		array_push($popular, $auction);
	}
} else {
	echo 'none';
}

$sql = "SELECT id, item_id, current_bid, `timestamp` FROM item_auction ORDER BY start_time ASC LIMIT 6"; // sql selecting newer auctions
$query = mysqli_query($db_connect, $sql);
$num_rows = mysqli_num_rows($query);
$newer=[];
if($num_rows > 0){
	while($row = mysqli_fetch_assoc($query)){
		$auction_id = $row['id'];
		$item_id = $row['item_id'];
		$current_bid = $row['current_bid'];
		$timestamp = $row['timestamp'];

		$sql = "SELECT name, bid_price, thumb FROM item WHERE id=$item_id";
		$query_2 = mysqli_query($db_connect, $sql);
		$row2 = mysqli_fetch_row($query_2);
		$name = $row2[0];
		$bid_price = $row2[1];
		$thumb = $row2[2];
		$auction = new auction;
		$auction -> auction_id = $auction_id;
		$auction -> item_id = $item_id;
		$auction -> name = $name;
		$auction -> bid_price = $bid_price;
		$auction -> current_bid = $current_bid;
		$auction -> timestamp = $timestamp;
		$auction -> thumbnail = $thumb;
		array_push($newer, $auction);
	}
} else {
	echo 'none';
}

$return = new return_data;
//$return -> completed = $completed;
$return -> popular = $popular;
$return -> newer = $newer;

echo json_encode($return);
exit();
?>